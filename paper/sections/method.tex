...
Each unit~$a_i$ is a structured record comprising a natural-language claim~$c_i$, a set of premise identifiers $P_i \subseteq \mathcal{A}\setminus\{a_i\}$ on which the claim depends, and a self-assessed confidence score $\gamma_i\in(0,1]$ that serves as an epistemic uncertainty proxy for the claim.
...
A \emph{contradiction} verdict yields an attack $(a_i,a_j)\in\mathcal{R}$, while an \emph{entailment} verdict records a support link used for downstream analysis but not encoded in~$\mathcal{R}$, since Dung-style frameworks model attacks only~\citep{dung1995acceptability}.
...
\textbf{ASP Encoding.}
We encode the repair problem as an answer set program following~\citet{egly2010asparg}, with choice atoms for candidate operations, semantics constraints enforcing~$\sigma$, and a weak constraint $\mathsf{\#minimize}\bigl\{\kappa(o) : \mathsf{selected}(o)\bigr\}$ selecting minimum-cost repairs.
Algorithm~\ref{alg:repair} summarizes the procedure.
When the solver selects $\mathsf{add\_arg}(a)$, the claim for the new argument is generated by prompting the LLM to rebut the target's attacker conditioned on~$\Delta$, then verified via NLI before admission.
...
Even under preferred semantics the repair problem is NP-complete (Theorem~\ref{thm:complexity}), rising to $\Sigma_2^P$-completeness under skeptical stable semantics~\citep{dvorak2018computational}, so we introduce two approximation strategies.
