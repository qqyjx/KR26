...
\textbf{Cost Function.}
Each operation~$o$ is assigned a non-negative cost $\kappa(o)\in\mathbb{R}_{\geq 0}$.
...
Under \emph{confidence-weighted cost}, argument deletions are weighted by the confidence of the removed argument, $\kappa(\mathsf{del\_arg}(a_i))=\gamma_i$, reflecting the intuition that highly confident claims should be more expensive to retract.
...
The repair problem under preferred semantics is $\Sigma_2^P$-complete in general~\cite{dvorak2012computational}, so we introduce two approximation strategies.
