...
Success follows directly from the validity constraint in Definition~\ref{def:repair}: any repair returned by the solver satisfies the prescribed status.  For inclusion, suppose some $\mathsf{del\_arg}(a)\in\mathit{Ops}^*$ were unnecessary for validity; removing it would yield a valid repair of strictly lower cost ($\kappa>0$), contradicting optimality---hence every deletion in $\mathit{Ops}^*$ is necessary, and all non-deleted elements are retained.  Vacuity is immediate: when no edits are needed, the empty set is valid and has cost zero, so no non-empty set can be cheaper.
\end{proof}
...
\begin{remark}
The reduction to enforcement establishes complexity bounds but does not subsume the repair problem.  Three structural differences distinguish our formulation: (i)~an evidence update~$\Delta$ first perturbs the framework before repair begins, coupling the search space to external input that enforcement does not model; (ii)~heterogeneous cost functions weight operations by argument confidence and structural role, whereas enforcement minimizes unweighted edit count; and (iii)~each $\mathsf{add\_arg}$ operation is grounded through the NLI pipeline (\S\ref{sec:relation}), constraining the candidate space by semantic plausibility rather than allowing arbitrary graph modifications.
\end{remark}
