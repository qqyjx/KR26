...
Under skeptical acceptance with stable semantics, membership in $\Sigma_2^P$ follows because one can guess a repair of cost at most~$C$ and then verify---via a co-NP oracle~\cite{dvorak2018computational}---that every stable extension of the repaired framework contains~$a_t$.  Hardness follows by polynomial reduction from the $\Sigma_2^P$-hard problem of deciding whether argument deletion can make a target skeptically stable-accepted~\cite{wallner2017complexity}; since $\mathsf{del\_arg}$ is one of our four repair operations, the lower bound carries over.
