...
Success follows directly from the validity constraint in Definition~\ref{def:repair}: any repair returned by the solver satisfies the prescribed status.  Inclusion holds because every deletion incurs a positive cost, so the optimizer never removes an element unless forced.  Vacuity is immediate: when no edits are needed, the empty sequence is valid and has cost zero, so no non-empty sequence can be cheaper.
\end{proof}
...
Success: after incorporating $\Delta = (\{a_5\}, \{(a_5, a_3)\}, \emptyset, \emptyset)$, $a_0$ reinstates and rejects~$a_4$; the repair $\langle \mathsf{add\_arg}(a_6), \mathsf{add\_att}(a_6, a_5) \rangle$ restores~$a_4$ to accepted status by defeating~$a_5$, which in turn restores~$a_3$ and re-defeats~$a_0$.
...
We note that inclusion, as stated, follows constructively from the semantics of $\mathsf{apply}$; the substantive minimal-change guarantee arises from cost optimization, which prevents unnecessary deletions when $\kappa > 0$.

We adapted three of the eight classical AGM postulates. The remaining five merit discussion.
\emph{Consistency} holds under preferred semantics, since every AF admits at least one preferred extension; under stable semantics, the solver reports infeasibility when no repair yields a framework with a stable extension.
\emph{Extensionality} holds because the repair operator is defined purely over the graph structure of the AF; syntactically identical inputs receive identical repairs.
The AGM postulates of \emph{closure} and \emph{recovery} do not transfer directly: closure assumes a deductively closed belief set, whereas AFs are graph structures without a deductive closure operation, and recovery presupposes that contraction followed by expansion restores the original beliefs---a property with no natural analogue when edits operate on arguments and attacks rather than logical sentences.
Similarly, \emph{superexpansion} and \emph{subexpansion} govern the interaction of revision with the conjunction of new information, a construct that does not arise when evidence updates consist of argument and attack additions and removals.
The three postulates we adapt---success, inclusion, and vacuity---capture the essential minimal-change desiderata for argumentation repair; a full axiomatic characterization of argumentation-specific revision is an interesting direction for future work.

\subsection{Computational Complexity}

The complexity of the repair problem depends critically on the choice of argumentation semantics. Our results assume credulous acceptance (as defined in \S\ref{sec:preliminaries}), inheriting the corresponding complexity landscape~\cite{dunne2009complexity,dvorak2012computational}.
...
\begin{proof}[Proof sketch]
For grounded semantics, the unique grounded extension is computable in polynomial time via the characteristic function~\cite{dung1995acceptability}, so verifying whether a candidate repair achieves the desired status is polynomial.  Because the grounded extension can be recomputed in polynomial time after each edit operation, the optimal repair can be found by a polynomial-time greedy procedure analogous to the enforcement algorithm of Baumann and Brewka~\cite{baumann2010complexity}.
...
Under skeptical acceptance, verifying that \emph{every} stable extension accepts the target is co-NP-hard~\cite{dvorak2012computational}, yielding $\Sigma_2^P$-completeness.
