...
Among the eight classical AGM postulates~\cite{katsuno1992update}, \emph{consistency} and \emph{extensionality} also hold (the former under preferred semantics; the latter because the operator is defined purely over graph structure), while \emph{closure}, \emph{recovery}, \emph{superexpansion}, and \emph{subexpansion} presuppose deductively closed belief sets or conjunctive information---constructs without natural analogues in argumentation frameworks.
The three postulates we adapt capture the essential minimal-change desiderata for LLM explanation maintenance.  The challenge is non-trivial: unlike classical belief revision where the input is a sentence, our setting requires reasoning about graph-structural changes where inclusion and vacuity interact with the combinatorial structure of extensions.
...
\begin{remark}
The reduction to enforcement establishes complexity bounds but does not subsume the repair problem: whereas enforcement seeks structural modifications to achieve a desired status, our formulation additionally incorporates an evidence update~$\Delta$ that first perturbs the framework, employs heterogeneous cost functions reflecting argument confidence and structural role, and operates within a pipeline that preserves the explanatory context of the original reasoning chain.
\end{remark}

These results motivate the $k$-neighborhood approximation introduced in \S\ref{sec:repair}: by restricting the search space, we reduce the effective problem size and ensure tractability even under preferred semantics for the framework sizes encountered in practice.  In practical terms, grounded repairs complete in under 0.2\,s and preferred repairs in under 0.5\,s for the framework sizes in our benchmarks (\S\ref{sec:experiments}).
