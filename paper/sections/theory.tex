...
The AGM theory of belief revision~\citep{alchourron1985agm} prescribes rationality postulates that any principled revision operator should satisfy.  We adapt three core postulates---success, inclusion, and vacuity---to the argumentation repair setting.  Intuitively, success requires that the repair achieves the desired outcome; inclusion requires that the repaired framework retains as much of the original as possible; and vacuity requires that no edits are made when the current state already satisfies the goal.
...
Among the remaining AGM postulates, consistency and extensionality also hold; recovery fails because structural additions cannot be automatically unwound by evidence retraction; and closure, superexpansion, and subexpansion lack natural analogues in graph-structural settings (details in Appendix~\ref{app:agm-extended}).
These three postulates \emph{characterize} the class of minimum-cost repair operators:
...
($\Leftarrow$) Given an operator satisfying the three postulates, define~$\kappa(o) = 1$ for every operation~$o$.  Success guarantees validity; vacuity ensures the empty set is returned when no repair is needed.  We claim the returned repair $\mathit{Ops}^*$ is cost-minimum.  Suppose not; then some cheaper valid repair $\mathit{Ops}'$ exists with $|\mathit{Ops}'| < |\mathit{Ops}^*|$.  Pick any operation $o \in \mathit{Ops}^* \setminus \mathit{Ops}'$: since $\mathit{Ops}'$ is valid without $o$, the operation $o$ can be dropped from $\mathit{Ops}^*$ while preserving validity, contradicting inclusion (which forbids unnecessary deletions under $\kappa > 0$).  Hence no cheaper valid repair exists, so $\mathit{Ops}^*$ is optimal.  The full construction for general~$\kappa$ appears in Appendix~\ref{app:representation}.
\end{proof}

\subsection{Computational Complexity}

The complexity of the repair problem depends critically on the choice of argumentation semantics.  Since the repair problem reduces to enforcement after incorporating~$\Delta$, it inherits the complexity landscape of extension enforcement~\citep{dunne2009complexity,dvorak2018computational}; the additional overhead of processing~$\Delta$ and evaluating heterogeneous cost functions is polynomial and does not alter the complexity class.  Our results assume credulous acceptance (as defined in \S\ref{sec:preliminaries}).
...
\begin{proof}[Proof sketch]
For grounded semantics, the unique grounded extension is computable in polynomial time via the characteristic function~\citep{dung1995acceptability}.  Membership in P follows by reduction to grounded enforcement, which \citet{dvorak2018computational} showed is solvable in polynomial time by exploiting the monotonicity of the characteristic function.  Our repair problem reduces to enforcement: we first incorporate the evidence update~$\Delta$ into the framework and then seek a minimum-cost set of edit operations that enforces the target argument's desired acceptability status; since both the incorporation of~$\Delta$ and the verification of any candidate repair via the grounded extension are polynomial, the overall decision problem is in~P.
For preferred semantics, hardness reduces from NP-hard extension enforcement~\citep{baumann2010complexity}; membership in NP follows since a valid repair paired with a witnessing admissible set can be guessed and verified in polynomial time.  Stable semantics under credulous acceptance inherits the same bounds; under skeptical acceptance, membership in $\Sigma_2^P$ follows by guessing a repair and verifying via a co-NP oracle that every stable extension contains~$a_t$, while hardness reduces from~\citet{wallner2017complexity}.
\end{proof}

Skeptical acceptance under preferred semantics yields a $\Pi_2^P$ lower bound by an analogous hardness reduction; we omit the full analysis for this case.  Note that the reduction to enforcement establishes complexity bounds but does not subsume the repair problem, which additionally involves evidence updates~$\Delta$, heterogeneous cost functions, and NLI-grounded candidate generation (\S\ref{sec:relation}).  These results motivate the $k$-neighborhood approximation (\S\ref{sec:repair}), ensuring tractability under preferred semantics for practical framework sizes.
...
The proof follows from the established correctness of the argumentation encodings of \citet{egly2010asparg} composed with the standard semantics of weak constraints in ASP solvers such as \emph{clingo}~\citep{gebser2019clingo}.  The composition is sound because the generate rules enumerate exactly the feasible edit operations and the integrity constraints enforce the semantics of the repaired framework, while the optimization directive selects minimum-cost solutions.
