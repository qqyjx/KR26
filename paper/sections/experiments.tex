...
The formal properties from \S\ref{sec:theory} are confirmed empirically: success and inclusion hold by construction; vacuity holds without exception (empty repair at zero cost whenever the update did not alter the target's status, covering 5\% of HotpotQA and 8\% of FEVER instances).  The constructed frameworks averaged 6.8 arguments (max~18) on HotpotQA and 5.4 (max~14) on FEVER, with defense sets averaging 2.4 and 2.1 arguments respectively.  Grounded-semantics solve times averaged 0.12s and preferred 0.43s per instance, well within practical bounds.
...
Figure~\ref{fig:scalability} traces solve time on synthetic Erd\H{o}s--R\'{e}nyi frameworks (attack probability 0.15, 50 instances per size), confirming polynomial scaling for grounded semantics (Theorem~\ref{thm:complexity}).  The $k$-neighborhood approximation keeps preferred repair tractable up to $|\mathcal{A}|{=}50$; unconstrained preferred repair exhibits exponential blowup beyond $|\mathcal{A}| \approx 25$.
...
Table~\ref{tab:ablation} reports an ablation on HotpotQA.  Removing semantic verification causes the largest drops in faithfulness and contestability.  Replacing minimal-change with unconstrained repair preserves faithfulness but increases cost from \resultRepairCostHotpot{} to 5.7, confirming that cost minimization limits unnecessary edits.  Removing attack templates reduces contestability by 9.3pp while leaving faithfulness intact.  Grounded-only semantics yields modest decreases across all metrics; the gap is small because most frameworks have a single preferred extension coinciding with the grounded one.

Sensitivity analysis, error analysis, repair cost distributions, and a qualitative example appear in Appendices~\ref{app:sensitivity}--\ref{app:repair-example}; the repair mechanism is robust to cost models, NLI thresholds, and $k$-neighborhood bounds, with 83\% of repairs requiring at most 4~operations.

A pilot human evaluation (Appendix~\ref{app:human-eval}) corroborates these findings: annotators preferred \textsc{Argus} in 68\% of blind pairwise comparisons ($\kappa{=}0.62$), and the automatic faithfulness metric correlates with human judgments ($r{=}0.78$, $p{<}0.001$).
