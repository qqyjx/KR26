...
We evaluate on 500 randomly sampled instances from HotpotQA~\cite{yang2018hotpotqa}, a multi-hop question-answering benchmark, and 500 from FEVER~\cite{thorne2018fever}, a fact-verification benchmark; instances are drawn with seed 42.
...
Results for \textsc{Argus} and the eight argumentation-based and self-correction methods are averaged over 5 runs, with standard deviation at most~0.02 for accuracy metrics and at most~0.4 for cost; FLARE and FactScore are evaluated via a single deterministic run of the published models on our instances.  Further details on the withholding methodology, hardware, and reproducibility are provided in Appendix~\ref{app:exp-details}.
...
For self-correction baselines, repair cost counts regenerated argument units across up to 3 rounds; cost measures are not directly commensurable with \textsc{Argus}'s structural graph edits, as discussed in Appendix~\ref{app:exp-details}.
...
Table~\ref{tab:main} summarizes the main results. \textsc{Argus} achieves the highest faithfulness (\resultFaithHotpot{}/\resultFaithFEVER{}) and contestability (\resultContestHotpot{}/\resultContestFEVER{}), with relative improvements of \improveFaithfulness{} in faithfulness and \improveContestability{} in contestability over ARGORA; all differences are statistically significant at $p < 0.001$ under a Bonferroni-corrected $z$-test.  Among repair-capable methods, \textsc{Argus} requires the fewest operations---\resultRepairCostHotpot{} vs.\ 5.1 for ARGORA---validating the minimal-change objective.  The two new retrieval-augmented baselines, FLARE and FactScore, achieve moderate faithfulness through improved evidence grounding but lack argumentation structure, resulting in low contestability scores of .505/.482 and .558/.535, respectively. FLARE's repair cost is the highest at 8.8/8.2 operations, due to iterative retrieval rounds without structural guidance.  The na\"{i}ve Regenerate baseline achieves the lowest faithfulness at .709/.695 and the fastest solve time at 0.5\,s, but cannot produce contestability or coherence scores, confirming that argumentation structure is essential for principled repair.

\textsc{Argus} also achieves the highest coherence at .82/.80, indicating that minimal-change repair preserves explanation structure better than rewriting-based methods such as Self-Refine, which scores .72/.70. We note that BERTScore coherence is structurally aligned with the minimal-change objective: minimizing structural edit distance to the original explanation simultaneously maximizes BERTScore similarity; the coherence advantage should therefore be interpreted as a formal property of \textsc{Argus}'s design rather than an independent empirical dimension. The average solve time of 0.55\,s on HotpotQA and 0.47\,s on FEVER is competitive with verification-only methods and 5--10$\times$ faster than self-correction methods that require multiple LLM rounds, which range from 2.8\,s to 5.8\,s, reflecting the efficiency of ASP-based repair over iterative LLM regeneration. The formal properties from \S\ref{sec:theory} are confirmed empirically: success and inclusion hold by construction of the ASP encoding; vacuity holds without exception, covering 5\% of HotpotQA and 8\% of FEVER instances where the withheld fact was not on the reasoning path. Grounded-semantics solve times averaged 0.12\,s and preferred semantics 0.43\,s per instance.
...
Table~\ref{tab:ablation} reports ablation results on both benchmarks; trends are consistent across datasets. Removing semantic verification causes the largest drops in faithfulness ($-$5.4pp/$-$5.4pp) and contestability ($-$7.7pp/$-$7.6pp), confirming that formal verification is the most critical component. Replacing minimal-change with unconstrained repair preserves faithfulness but increases cost from \resultRepairCostHotpot{}/\resultRepairCostFEVER{} to 5.7/5.2 and reduces coherence, confirming that cost minimization limits unnecessary edits. Removing attack templates reduces contestability by 9.3pp/9.0pp while reducing faithfulness by only 2.6pp/2.5pp---a targeted impact specific to attack structure. Grounded-only semantics yields the fastest solve time---0.15\,s/0.12\,s---and the lowest repair cost at 3.0/2.6 vs.\ 3.2/2.8 for Full \textsc{Argus}, at the expense of modest faithfulness and contestability decreases. The lower cost is expected: grounded semantics imposes stronger constraints that produce smaller, more conservative repair sets; the cost-quality gap is small because 97\% of frameworks have a single preferred extension coinciding with the grounded one, confirming that preferred semantics is necessary only in complex multi-extension scenarios. Sensitivity analysis and a qualitative repair example appear in Appendices~\ref{app:sensitivity}--\ref{app:repair-example}.
...
A pilot human evaluation corroborates the automatic metrics; details appear in Appendix~\ref{app:human-eval}. Two annotators rated 75 HotpotQA instances in a blind design, preferring \textsc{Argus} in 68\% of comparisons vs.\ Self-Refine in 19\%, with inter-annotator agreement of $\kappa{=}0.62$. \textsc{Argus} received higher faithfulness---3.9 vs.\ 3.4 on a 5-point Likert scale at $p < 0.001$---and higher coherence ratings of 4.1 vs.\ 3.8 at $p = 0.012$. The Pearson correlation between automatic faithfulness and human ratings is $r{=}0.78$ at $p{<}0.001$, supporting counterfactual ablation as a proxy for human-perceived faithfulness.
