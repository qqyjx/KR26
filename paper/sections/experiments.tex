...
Varying the NLI threshold from 0.5 to 0.9 on the same pilot subset shows faithfulness is stable (0.839--0.851) while repair cost rises from 2.4 to 4.1 as fewer attacks are admitted; 0.7 balances these factors.
Similarly, repair optimality rises from 87.2\% ($k{=}1$) to 99.7\% ($k{=}3$) and plateaus, confirming $k{=}3$ as the operating point.
...
Faithfulness reached 0.813 and contestability 0.762, compared with \resultFaithHotpot{} and \resultContestHotpot{} for GPT-4o on the same subset.
