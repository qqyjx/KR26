...
The repair operator draws on two classical KR traditions: the AGM theory of belief revision~\cite{alchourron1985agm}, which supplies the minimal-change principle, and argumentation dynamics~\cite{cayrol2020argumentation}, which provides formal machinery for structural change.
...
\begin{enumerate}
    ...
    \item \textbf{(C3)} A scalable ASP encoding with a $k$-neighborhood approximation that preserves repair quality while substantially reducing solver grounding (\S\ref{sec:method}).
