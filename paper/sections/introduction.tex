...
\centering
\begin{tikzpicture}[
  method/.style={rounded corners=3pt, inner sep=3pt, font=\small,
                 line width=0.5pt},
]
  % Background
  \fill[gray!4, rounded corners=2pt] (-0.3, -0.3) rectangle (5.0, 3.4);
  \foreach \x in {1,2,3,4} \draw[gray!12, thin] (\x, -0.1) -- (\x, 3.2);
  \foreach \y in {1,2,3} \draw[gray!12, thin] (-0.1, \y) -- (4.8, \y);
  % Axes
  \draw[-{Stealth[length=3mm, width=2mm]}, black!60, line width=0.9pt]
        (-0.2, 0) -- (4.9, 0);
  \draw[-{Stealth[length=3mm, width=2mm]}, black!60, line width=0.9pt]
        (0, -0.2) -- (0, 3.4);
  \node[font=\small, black!70] at (4.9, -0.4) {Formal guarantees};
  \node[font=\small, black!70, rotate=90, anchor=south] at (-0.55, 1.5)
        {Repair capability};
  % ARGUS highlight region
  \fill[ArgusMain!8, rounded corners=4pt] (2.3, 1.6) rectangle (4.6, 3.15);
  \draw[ArgusMain!30, dashed, line width=0.6pt, rounded corners=4pt]
        (2.3, 1.6) rectangle (4.6, 3.15);
  % Self-correction methods (orange)
  \node[method, fill=SelfCorr!25, draw=SelfCorr!60!black]
        at (0.7, 2.7) {Self-Refine};
  \node[method, fill=SelfCorr!25, draw=SelfCorr!60!black]
        at (0.9, 2.15) {Reflexion};
  \node[method, fill=SelfCorr!25, draw=SelfCorr!60!black]
        at (1.4, 1.7) {RARR};
  % Argumentation methods (green)
  \node[method, fill=ArguMeth!20, draw=ArguMeth!60!black]
        at (3.8, 0.5) {ArgLLMs};
  \node[method, fill=ArguMeth!20, draw=ArguMeth!60!black]
        at (3.0, 1.1) {ARGORA};
  \node[method, fill=ArguMeth!20, draw=ArguMeth!60!black]
        at (2.3, 0.4) {CoT-Verifier};
  % ARGUS (dominant)
  \node[method, fill=ArgusMain!30, draw=ArgusMain!80!black,
        line width=1.2pt, font=\small\bfseries]
        at (3.5, 2.4) {\textsc{Argus}};
\end{tikzpicture}
...
Self-correction methods~\cite{madaan2023selfrefine,shinn2023reflexion,gao2023rarr} iteratively rewrite explanations but without formal guarantees---edits are unconstrained and previously valid reasoning may be silently discarded.
Argumentation-based approaches~\cite{freedman2025arglm,argora2026} verify explanations against formal semantics but treat verification as terminal: when new evidence arrives, they offer no principled way to update the explanation while preserving consistency.
...
\begin{figure*}[t]
\centering
\subcaptionbox{$F_0$: Initial framework ($a_4$ accepted)\label{fig:af-f0}}[0.32\textwidth]{%
\begin{tikzpicture}[node distance=0.9cm and 1.0cm]
  ...
  \node[rej node, below=of a4] (a0) {$a_0$};
  \draw[att edge] (a0) -- (a4);
  \draw[att edge] (a3) -- (a0);
\end{tikzpicture}%
}\hfill
\subcaptionbox{$F_1$: After evidence update ($a_4$ rejected)\label{fig:af-f1}}[0.32\textwidth]{%
\begin{tikzpicture}[node distance=0.9cm and 1.0cm]
  \node[rej node, tgt node] (a4) {$a_4$};
  \node[rej node, left=of a4] (a3) {$a_3$};
  \node[acc node, above=of a3] (a1) {$a_1$};
  \node[acc node, above=of a4] (a2) {$a_2$};
  \node[acc node, below=of a4] (a0) {$a_0$};
  \node[new node, above right=0.5cm and 0.6cm of a2] (a5) {$a_5$};
  \draw[att edge] (a0) -- (a4);
  \draw[att edge] (a3) -- (a0);
  \draw[att edge] (a5) -- (a3);
\end{tikzpicture}%
}\hfill
\subcaptionbox{$F_2$: After repair ($a_4$ restored)\label{fig:af-f2}}[0.32\textwidth]{%
\begin{tikzpicture}[node distance=0.9cm and 1.0cm]
  \node[acc node, tgt node] (a4) {$a_4$};
  \node[acc node, left=of a4] (a3) {$a_3$};
  \node[acc node, above=of a3] (a1) {$a_1$};
  \node[acc node, above=of a4] (a2) {$a_2$};
  \node[rej node, below=of a4] (a0) {$a_0$};
  \node[rej node, above right=0.5cm and 0.6cm of a2] (a5) {$a_5$};
  \node[new node, right=0.6cm of a5] (a6) {$a_6$};
  \draw[att edge] (a0) -- (a4);
  \draw[att edge] (a3) -- (a0);
  \draw[att edge] (a5) -- (a3);
...
\begin{enumerate}
    ...
    \item \textbf{(C2)} A minimal-change repair operator for explanation maintenance under evolving evidence, satisfying adapted AGM revision postulates with a complexity analysis placing the problem in P under grounded semantics and NP-complete under preferred and stable semantics (\S\ref{sec:repair}--\S\ref{sec:theory}).
