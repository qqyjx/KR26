...
\centering
\begin{tikzpicture}[
  method/.style={rounded corners=2pt, inner sep=2pt, font=\scriptsize},
]
  \draw[-{Stealth[length=2mm]}, gray!70] (-0.2, 0) -- (4.6, 0);
  \draw[-{Stealth[length=2mm]}, gray!70] (0, -0.2) -- (0, 3.2);
  \node[font=\scriptsize, gray!60!black] at (4.6, -0.35) {Formal guarantees};
  \node[font=\scriptsize, gray!60!black, rotate=90, anchor=south] at (-0.45, 1.5) {Repair capability};
  \fill[blue!6, rounded corners=3pt] (2.4, 1.7) rectangle (4.4, 3.0);
  \node[method, fill=orange!20, draw=orange!40] at (0.7, 2.6) {Self-Refine};
  \node[method, fill=orange!20, draw=orange!40] at (0.9, 2.1) {Reflexion};
  \node[method, fill=orange!20, draw=orange!40] at (1.3, 1.7) {RARR};
  \node[method, fill=green!15, draw=green!40] at (3.7, 0.5) {ArgLLMs};
  \node[method, fill=green!15, draw=green!40] at (2.9, 1.1) {ARGORA};
  \node[method, fill=green!15, draw=green!40] at (2.3, 0.4) {CoT-Verifier};
  \node[method, fill=blue!25, draw=blue!60, thick, font=\scriptsize\bfseries] at (3.5, 2.4) {\textsc{Argus}};
\end{tikzpicture}
\caption{Positioning of \textsc{Argus}. Self-correction methods (orange) repair without formal guarantees; argumentation-based methods (green) verify without principled repair. \textsc{Argus} (blue region) bridges both dimensions.}
...
\begin{figure*}[t]
\centering
\subcaptionbox{$F_0$: Initial framework ($a_4$ accepted)\label{fig:af-f0}}[0.30\textwidth]{%
