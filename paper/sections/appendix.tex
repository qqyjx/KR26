...
\textbf{Construction.}  Fix an enumeration of all feasible operations $o_1, \ldots, o_m$ for the given AF and $\Delta$.  Define $\kappa(o_i) = 1$ for all $i$.  Let $\mathit{Ops} = \circ(\mathcal{A}, \mathcal{R}, \sigma, a_t, s, \Delta)$.

\textbf{Validity.}  By success, $a_t$ has status $s$ in $\mathsf{apply}(\mathcal{A}, \mathcal{R}, \Delta, \mathit{Ops})$ under $\sigma$, so $\mathit{Ops}$ is a valid repair.

\textbf{Vacuity case.}  If $a_t$ already has status $s$ in $\mathsf{apply}(\mathcal{A}, \mathcal{R}, \Delta, \emptyset)$, then by vacuity $\mathit{Ops} = \emptyset$ with cost $0$, which is trivially minimum.
...
Faithfulness is measured via counterfactual ablation: each argument unit is replaced with a semantically neutral sentence (``This claim is omitted.'') and the answer is regenerated; a unit is faithful if its removal changes the answer.
...
Gold counterarguments for contestability are derived from the withheld supporting facts by expressing each as an argument and annotating the expected attack relationships, providing a ground truth independent of the repair mechanism.
